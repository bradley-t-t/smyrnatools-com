...
async function getNotifications({userId, selectedRegion}) {
  ...
  const rmiNode=await UserService.hasPermission(userId,'notifications.rmi').catch(()=>false)
  ...
  if(!rmiNode && !dmNode && !gmNode) return []
  ...
  try {
    ...
    if (rmiNode && !submittedUserIds.has(userId)) {
      notifications.push({
        id:`reports-overdue-rmi-${userId}`,
        title:'Ready Mix Instructor Report Overdue',
        subtitle:`Week ${weekRangeStr}`,
        severity:'error',
        type:'reports.overdue.ready_mix_instructor'
      })
    }
    ...
    if (dmNode || gmNode) {
      const {data: users} = await supabase
        ...
        .contains('permission_nodes', ['reports.assigned.rmi'])
      
      if (users && users.length > 0) {
        for (const user of users) {
          if (!submittedUserIds.has(user.id)) {
            notifications.push({
              id:`reports-overdue-rmi-user-${user.id}`,
              title:`${user.first_name} ${user.last_name}'s RMI Report Overdue`,
              subtitle:`Week ${weekRangeStr}`,
              severity:'error',
              type:'reports.overdue.ready_mix_instructor'
            })
...
export default {id:'reports.overdue.ready_mix_instructor', getNotifications}
