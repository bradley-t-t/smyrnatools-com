...
async function getNotifications({userId, selectedRegion}) {
  ...
  try {
    const {data,error} = await supabase
      ...
      .select('*')
      .eq('week',weekIso)
      .eq('report_name','district_manager')
    ...
    if (dmNode && !submittedUserIds.has(userId)) {
      notifications.push({
        id:`reports-overdue-dm-${userId}`,
        title:'District Manager Report Overdue',
        subtitle:`Week ${weekRangeStr}`,
        severity:'error',
        type:'reports.overdue.district_manager'
      })
    }
    ...
    if (gmNode) {
      const {data: users} = await supabase
        ...
        .contains('permission_nodes', ['reports.assigned.district_manager'])
      
      if (users && users.length > 0) {
        for (const user of users) {
          if (!submittedUserIds.has(user.id)) {
            notifications.push({
              id:`reports-overdue-dm-user-${user.id}`,
              title:`${user.first_name} ${user.last_name}'s DM Report Overdue`,
              subtitle:`Week ${weekRangeStr}`,
              severity:'error',
              type:'reports.overdue.district_manager'
            })
...
export default {id:'reports.overdue.district_manager', getNotifications}
