...
export class BaseService {
    ...
    map(row) {
        return this.ModelClass ? new this.ModelClass(row) : row
    }

    _handleError(op, error, context = {}) {
        ErrorUtility.logError(`${this.tableName}.${op}`, error, context)
        throw error
    }
    ...
    _cacheSet(key, value) {
        CacheUtility.set(key, value, this.cacheTtl)
        return value
    }
    ...
    async getById(id, { select = '*', useCache = true } = {}) {
        try {
            ...
            this._handleError('getById', e, { id })
        }
    }
    ...
    async create(payload, { returning = true } = {}) {
        try {
            ...
            this._handleError('create', e, { payload })
        }
    }
    ...
    async update(id, payload, { returning = true } = {}) {
        try {
            ...
            this._handleError('update', e, { id, payload })
        }
    }
