...
export function GeneralManagerSubmitPlugin({form, setForm, plants = [], readOnly, weekIso}) {
    ...
    React.useEffect(() => {
        ...
        async function loadEff() {
            ...
            windowEnd.setUTCDate(windowEnd.getUTCDate() + 8) // include next Monday plus cushion day
...
export function GeneralManagerReviewPlugin({form, plants = [], weekIso}) {
    ...
    return (
        <div className="rpt-card rpt-card-accent">
            ...
            {included.length > 0 && (<div className="rpt-card-header">
                <div className="rpt-card-title">Per-Plant Summary</div>
                    ...
                    {included.map(p => {
                        ...
                        return (
                            <div className="rpt-card rpt-p-16" key={code}>
                                ...
                                <div className="rpt-form-row"><Field label="# of Operators" value={form[f.ops] ?? ''}
                                                                     onChange={() => {
                                                                     }} required disabled/><Field
                                    label="# of Runnable Trucks" value={form[f.runnable] ?? ''} onChange={() => {
                                }} required disabled/><Field label="Down Trucks" value={form[f.down] ?? ''}
                                                             onChange={() => {
                                                             }} required disabled/></div>
                                <div className="rpt-form-row"><Field label="Operators Starting"
                                                                     value={form[f.starting] ?? ''} onChange={() => {
                                }} required disabled/><Field label="Operators Leaving" value={form[f.leaving] ?? ''}
                                                             onChange={() => {
                                                             }} required disabled/><Field label="New Operators Training"
                                                                                          value={form[f.training] ?? ''}
                                                                                          onChange={() => {
                                                                                          }} required disabled/><Field
                                    label="Total Yardage" value={form[f.yardage] ?? ''} onChange={() => {
                                }} required disabled/></div>
                                <div className="rpt-form-row"><Field label="Total Hours" value={form[f.hours] ?? ''}
                                                                     onChange={() => {
                                                                     }} required disabled/></div>
                                {form[f.notes] ? (<div className="rpt-form-row"><Field label="Notes" type="textarea"
                                                                                       value={form[f.notes] ?? ''}
                                                                                       onChange={() => {
            ...
            <div className="rpt-card-header rpt-mt-16">
                <div className="rpt-card-title">Plant Efficiency Reports</div>
                {effReports.length > 0 && (<div className="rpt-badge">{effIdx + 1} of {effReports.length}</div>)}</div>
            {effReports.length === 0 ? (
                <div className="rpt-empty">No plant efficiency reports found for this week.</div>) : (
                <div className="rpt-form-row rpt-flex-col">
                    <div className="rpt-dots-bar">{effReports.map((r, i) => (
                        <div key={r.id} onClick={() => setEffIdx(i)}
                             className={`rpt-dot ${i === effIdx ? 'active' : ''}`}
                             aria-label={`Efficiency Report ${i + 1}`}></div>))}</div>
                    {(() => {
                        const r = effReports[effIdx];
                        const insights = ReportService.getPlantProductionInsights(r.rows || []);
                        return (
