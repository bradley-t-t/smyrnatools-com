...
async function createWeekSheet(wb, ExcelLib, form, plants, weekIso, prevGMData, prevWeekIso, allMonthlyData, logoBase64, assetData) {
    ...
    if (sortedEffReports.length > 0) {
        ...
        const effLoadsChange = prevEffCount > 0 ? getChangeText(totalLoadsEff, prevTotalLoadsEff, false) : {
            text: '',
            color: null
        }
        const effHoursChange = prevEffCount > 0 ? getChangeText(totalHoursEff, prevTotalHoursEff, false) : {
            text: '',
            color: null
        }
