...
const RPT_INPUT =
    'w-full rounded-md border border-gray-200 bg-white px-3.5 py-2.5 text-sm text-slate-800 box-border disabled:bg-slate-50 disabled:text-slate-500 focus:outline-none focus:border-[#1e3a5f] focus:ring-2 focus:ring-[#1e3a5f]/10'
...
export function ReportCard({ title, accent, badge, actions, children, className = '' }) {
    return (
        <div
            className={`rounded-xl border border-gray-200 bg-white p-6 mb-6 ${accent ? 'border-l-4 border-l-[#1e3a5f]' : ''} ${className}`}
        >
