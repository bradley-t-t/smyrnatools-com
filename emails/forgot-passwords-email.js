export function buildForgotPasswordEmail({newPassword, loginUrl, theme, logoUrl}) {
    ...
    const styles = {
        ...
        pwdLabel: `font-size: 13px; ${css.color(t.textMuted)} text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600; margin: 0 0 12px; text-align: center;`.trim(),
        pwdBoxTable: `${css.bg("#fafbfc")} ${css.border(t.border)} border-radius: 8px; padding: 20px 24px; text-align: center;`.trim(),
        ...
        warningBox: `${css.bg("#fff5f5")} border-left: 4px solid ${t.warning}; padding: 16px 20px; border-radius: 6px; margin: 24px 0;`.trim(),
