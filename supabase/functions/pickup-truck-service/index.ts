        ...
        switch (endpoint) {
            ...
            case 'create': {
                ...
                const apiData: Record<string, any> = {
                    ...
                    year:
                        typeof pickup?.year === 'number'
                            ? pickup.year
                            : typeof pickup?.year === 'string'
                              ? pickup.year
                              : null,
                    assigned:
                        typeof pickup?.assigned === 'string'
                            ? pickup.assigned.trim() === ''
                                ? null
                                : pickup.assigned
                            : null,
                ...
                if (data?.id)
                    await supabase.from(HISTORY_TABLE).insert({
                        pickup_truck_id: data.id,
                        field_name: 'created',
                        old_value: null,
                        new_value: 'Pickup Truck created',
                        changed_at: now,
                        changed_by: userId
                    })
            ...
            case 'update': {
                ...
                const body = await parseBody(req)
                const id =
                    typeof body?.pickupId === 'string' ? body.pickupId : typeof body?.id === 'string' ? body.id : null
                ...
                const normalizedAssigned = has('assigned')
                    ? pickup.assigned == null || (typeof pickup.assigned === 'string' && pickup.assigned.trim() === '')
                        ? null
                        : String(pickup.assigned)
                    : current.assigned
                const normalizedAssignedPlant = has('assignedPlant')
                    ? pickup.assignedPlant == null
                        ? null
                        : String(pickup.assignedPlant)
                    : current.assigned_plant
                const normalizedStatus = has('status')
                    ? pickup.status == null
                        ? current.status
