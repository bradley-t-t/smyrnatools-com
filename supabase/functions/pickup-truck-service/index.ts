...
Deno.serve(async (req) => {
    ...
    try {
        ...
        switch (endpoint) {
            ...
            case "delete":
                return handleDelete(supabase, await parseBody(req), req, MAIN_TABLE, HISTORY_TABLE, HISTORY_ID_KEY, "Pickup Truck", headers);
            case "search-by-vin": {
                const auth = await requireAuthenticated(supabase, req, headers); if (auth instanceof Response) return auth;
            ...
            case "add-issue":
                return handleAddIssue(supabase, await parseBody(req), req, MAINTENANCE_TABLE, HISTORY_ID_KEY, "pickupId", headers);
            case "complete-issue":
