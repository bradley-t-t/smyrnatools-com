...
Deno.serve(async (req) => {
    ...
    try {
        ...
        switch (endpoint) {
            ...
            case "delete":
                return handleDelete(supabase, await parseBody(req), req, MAIN_TABLE, HISTORY_TABLE, ID_KEY, "Equipment", headers);
            case "fetch-comments": {
                const auth = await requireAuthenticated(supabase, req, headers); if (auth instanceof Response) return auth;
            ...
            case "add-comment":
                return handleAddComment(supabase, await parseBody(req), req, {main: MAIN_TABLE, history: HISTORY_TABLE, idKey: "equipmentId", comments: COMMENTS_TABLE}, headers);
            case "delete-comment":
            ...
            case "add-issue":
                return handleAddIssue(supabase, await parseBody(req), req, MAINTENANCE_TABLE, ID_KEY, "equipmentId", headers);
            case "complete-issue":
