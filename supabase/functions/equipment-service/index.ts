...
function getUserFriendlyError(error: string): string {
    ...
    if (lower.includes('year_made_check') || lower.includes('heavy_equipment_year_made_check'))
        return 'Year must be a valid year.'
    if (
        (lower.includes('duplicate key') && lower.includes('identifying_number')) ||
        lower.includes('heavy_equipment_identifying_number_key')
    )
...
async function enrichEquipment(supabase: any, data: any, id: string): Promise<Record<string, any>> {
    ...
    return {
        ...data,
        latestHistoryDate: hist?.[0]?.changed_at ?? null,
        ...
        switch (endpoint) {
            ...
            case 'create': {
                ...
                const apiData: Record<string, any> = {
                    ...
                    hours_mileage:
                        equipment?.hoursMileage != null
                            ? Number(equipment.hoursMileage)
                            : equipment?.hours_mileage != null
                              ? Number(equipment.hours_mileage)
                              : null,
                    cleanliness_rating:
                        equipment?.cleanlinessRating != null
                            ? Number(equipment.cleanlinessRating)
                            : equipment?.cleanliness_rating != null
                              ? Number(equipment.cleanliness_rating)
                              : null,
                    condition_rating:
                        equipment?.conditionRating != null
                            ? Number(equipment.conditionRating)
                            : equipment?.condition_rating != null
                              ? Number(equipment.condition_rating)
                              : null,
                    ...
                    year_made:
                        equipment?.yearMade != null
                            ? Number(equipment.yearMade)
                            : equipment?.year_made != null
                              ? Number(equipment.year_made)
                              : null,
            ...
            case 'update': {
                ...
                const apiData: Record<string, any> = {
                    ...
                    assigned_operator:
                        'assignedOperator' in equipment ? equipment.assignedOperator : current.assigned_operator,
                    ...
                    last_service_date:
                        'lastServiceDate' in equipment
                            ? toDbTimestamp(equipment.lastServiceDate)
                            : current.last_service_date,
                    hours_mileage: 'hoursMileage' in equipment ? Number(equipment.hoursMileage) : current.hours_mileage,
                    cleanliness_rating:
                        'cleanlinessRating' in equipment
                            ? Number(equipment.cleanlinessRating)
                            : current.cleanliness_rating,
                    condition_rating:
                        'conditionRating' in equipment ? Number(equipment.conditionRating) : current.condition_rating,
                    ...
                    updated_last:
                        typeof equipment?.updatedLast === 'string' ? equipment.updatedLast : current.updated_last
                }
            ...
            case 'fetch-comments': {
                ...
                return handleFetchComments(
                    supabase,
                    await parseBody(req),
                    { main: MAIN_TABLE, history: HISTORY_TABLE, idKey: 'equipmentId', comments: COMMENTS_TABLE },
                    headers
                )
            }
            case 'add-comment':
            ...
            case 'verify': {
                ...
                const { data, error } = await supabase
                    .from(MAIN_TABLE)
                    .update({ updated_last: nowIso(), updated_by: userId })
                    .eq('id', id)
                    .select()
                    .maybeSingle()
                if (error) return errorResponse('Operation failed', headers, 400)
