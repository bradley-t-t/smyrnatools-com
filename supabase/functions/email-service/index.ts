...
async function sendEmail({
    ...
    text,
    toList,
    ccList = [] as Array<{ email: string; name?: string }>,
    bccList = [] as Array<{ email: string; name?: string }>,
    debugMode = false,
    ...
    switch (endpoint) {
        ...
        case 'notify-report-submitted': {
            ...
            try {
                ...
                const [{ data: submitterProfile, error: profileErr }, { data: submitterUser, error: userErr }] =
                    await Promise.all([
                        supabase
                            .from('users_profiles')
                ...
                if (!submitterProfile?.plant_code || !submitterUser?.email) {
                    ...
                    await reportEmailError('Submitter profile or plant not found', {
                        userId,
                        reportTitle,
                    ...
                    return jsonResponse(
                        { success: false, sent: false, reason: 'Submitter profile or plant not found' },
                        headers
                    )
                }

                const submitterName =
                    [submitterProfile.first_name, submitterProfile.last_name].filter(Boolean).join(' ') ||
                    'A team member'
                ...
                if (!regionLink?.region_id) {
                    ...
                    await reportEmailError("No region found for submitter's plant", {
                        userId,
                        plantCode: submitterProfile.plant_code,
                        regionLinkErr: regionLinkErr?.message
                    })
                    return jsonResponse(
                        { success: false, sent: false, reason: "No region found for submitter's plant" },
                        headers
                    )
                }
                ...
                if (!region) {
                    ...
                    await reportEmailError('Region record not found', {
                        userId,
                ...
                if (!gmRole) {
                    ...
                    await reportEmailError('General Manager role not found in users_roles', {
                        userId,
                        gmRoleErr: gmRoleErr?.message
                    })
                    return jsonResponse(
                        { success: false, sent: false, reason: 'General Manager role not found' },
                        headers
                    )
                }
                ...
                console.log('[notify-report-submitted] Step 6 - GM profiles', {
                    ...
                    profiles: (gmProfiles || []).map((gm) => ({
                        id: gm.id,
                        plant_code: gm.plant_code,
                        inRegion: gm.plant_code ? regionPlantCodes.has(gm.plant_code) : false
                    })),
                ...
                console.log("[notify-report-submitted] Step 6b - GMs in submitter's region", {
                    count: regionGmIds.length,
                    ids: regionGmIds
                })

                if (regionGmIds.length === 0) {
                    console.error(
                        "[notify-report-submitted] STOP: no GMs have a plant_code that matches the submitter's region"
                    )
                    ...
                    return jsonResponse(
                        { success: false, sent: false, reason: "No General Managers in submitter's region" },
                        headers
                    )
                }
                ...
                if (dmUserIds.length > 0) {
                    const [{ data: dmProfiles, error: dmProfErr }, { data: dmUsers, error: dmUsersErr }] =
                        await Promise.all([
                    ...
                    const dmEmailMap = new Map(
                        (dmUsers || []).map((u: { id: string; email: string }) => [u.id, u.email])
                    )
                ...
                const ccRecipients = [...dmRecipients, { email: submitterEmail, name: submitterName }]
                ...
                if (debugMode) {
                    ...
                    if (!debugEmail) {
                        return jsonResponse(
                            { success: false, sent: false, reason: 'EMAIL_DEBUG_WHITELIST not set' },
                            headers
                        )
                    }
                    debugInfo = {
                ...
                if (attachmentUrl && typeof attachmentUrl === 'string') {
                    ...
                    if (base64Content) {
                        ...
                        console.warn(
                            '[notify-report-submitted] Step 12 - PDF fetch returned null, sending without attachment'
                        )
                    }
                }

                console.log('[notify-report-submitted] Step 13 - calling sendEmail', {
                    debugMode,
                    finalTo: finalToList.map((r) => r.email)
                })
                ...
                if (!result.success) {
                    await reportEmailError(`sendEmail returned failure: ${result.reason ?? 'unknown'}`, {
                        userId,
                        toCount: toRecipients.length,
                        debugMode
                    })
                ...
                await reportEmailError(`Unhandled exception: ${(err as Error)?.message ?? String(err)}`, {
                    userId,
                    reportTitle
                })
        ...
        case 'notify-comment-added': {
            ...
            try {
                    ...
                    if (pmUserIds.length > 0) {
                        ...
                        const matchingPmIds = (pmProfiles || [])
                            .filter((p: any) => {
                                ...
                                const additional = Array.isArray(p.additional_assigned_plants)
                                    ? p.additional_assigned_plants
                                    : []
                        ...
                        if (matchingPmIds.length > 0) {
                            ...
                            const optedOut = new Set(
                                (prefs || [])
                                    .filter((p: any) => p.accept_comment_emails === false)
                                    .map((p: any) => p.user_id)
                            )
                ...
                console.log('[email-service] Sending comment-added notifications', {
