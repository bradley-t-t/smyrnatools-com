        ...
        switch (endpoint) {
            case 'fetch-all': {
                ...
                try {
                    ...
                    Object.assign(latestMap, buildLatestMap(hist, ID_KEY))
                } catch {}
                try {
                    ...
                    Object.assign(issuesMap, buildCountMap(openIssues, ID_KEY))
                } catch {}
                try {
                    const { data: comments } = await supabase.from(COMMENTS_TABLE).select('tractor_id')
                    Object.assign(commentsMap, buildCountMap(comments, ID_KEY))
                } catch {}
                return jsonResponse(
                    {
                        data: (data || []).map((m: any) => ({
            ...
            case 'create': {
                ...
                const apiData: Record<string, any> = {
                    ...
                    cleanliness_rating:
                        typeof tractor?.cleanlinessRating === 'number'
                            ? tractor.cleanlinessRating
                            : typeof tractor?.cleanliness_rating === 'number'
                              ? tractor.cleanliness_rating
                              : 0,
                    has_blower:
                        typeof tractor?.hasBlower === 'boolean'
                            ? tractor.hasBlower
                            : typeof tractor?.has_blower === 'boolean'
                              ? tractor.has_blower
                              : null,
            ...
            case 'update': {
                ...
                const body = await parseBody(req)
                const id =
                    typeof body?.tractorId === 'string' ? body.tractorId : typeof body?.id === 'string' ? body.id : null
                ...
                const apiData: Record<string, any> = {
                    ...
                    last_service_date:
                        'lastServiceDate' in tractor
                            ? toDbTimestamp(tractor.lastServiceDate)
                            : current.last_service_date,
                    cleanliness_rating:
                        'cleanlinessRating' in tractor
                            ? typeof tractor.cleanlinessRating === 'number'
                                ? tractor.cleanlinessRating
                                : Number(tractor.cleanlinessRating)
                            : current.cleanliness_rating,
                    has_blower:
                        'hasBlower' in tractor
                            ? typeof tractor.hasBlower === 'boolean'
                                ? tractor.hasBlower
                                : Boolean(tractor.hasBlower)
                            : current.has_blower,
                    ...
                    freight:
                        'freight' in tractor
                            ? typeof tractor.freight === 'string'
                                ? tractor.freight
                                : String(tractor.freight)
                            : current.freight,
                    status,
            ...
            case 'fetch-comments': {
                ...
                return handleFetchComments(
                    supabase,
                    await parseBody(req),
                    { main: MAIN_TABLE, history: HISTORY_TABLE, idKey: 'tractorId', comments: COMMENTS_TABLE },
                    headers
                )
            }
            case 'add-comment':
