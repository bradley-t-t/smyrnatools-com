    ...
    try {
        ...
        switch (endpoint) {
            case 'fetch-all': {
                ...
                return jsonResponse(
                    { data: (data || []).map((m: any) => ({ ...m, latestHistoryDate: latestMap[m.id] ?? null })) },
                    headers
                )
            }
            case 'fetch-by-id': {
            ...
            case 'create': {
                ...
                const apiData: Record<string, any> = {
                    ...
                    cleanliness_rating:
                        typeof mixer?.cleanlinessRating === 'number'
                            ? mixer.cleanlinessRating
                            : typeof mixer?.cleanliness_rating === 'number'
                              ? mixer.cleanliness_rating
                              : 0,
                    vin: mixer?.vin ?? null,
            ...
            case 'update': {
                ...
                const body = await parseBody(req)
                const id =
                    typeof body?.mixerId === 'string' ? body.mixerId : typeof body?.id === 'string' ? body.id : null
                ...
                const apiData: Record<string, any> = {
                    ...
                    last_service_date:
                        'lastServiceDate' in mixer ? toDbTimestamp(mixer.lastServiceDate) : current.last_service_date,
                    last_chip_date:
                        'lastChipDate' in mixer ? toDbTimestamp(mixer.lastChipDate) : current.last_chip_date,
                    cleanliness_rating:
                        'cleanlinessRating' in mixer
                            ? typeof mixer.cleanlinessRating === 'number'
                                ? mixer.cleanlinessRating
                                : Number(mixer.cleanlinessRating)
            ...
            case 'fetch-comments': {
                ...
                return handleFetchComments(
                    supabase,
                    await parseBody(req),
                    { main: MAIN_TABLE, history: HISTORY_TABLE, idKey: 'mixerId', comments: COMMENTS_TABLE },
                    headers
                )
            }
            case 'add-comment':
            ...
            case 'upload-image': {
                ...
                if (!fileName || !fileBase64)
                    return errorResponse('File name and base64 content are required', headers, 400)
                const pathInBucket = `mixer_images/${fileName}`
                const { error: uploadError } = await supabase.storage
                    .from('smyrna')
                    .upload(pathInBucket, decodeBase64ToUint8Array(fileBase64), { contentType })
                if (uploadError) return errorResponse('Operation failed', headers, 400)
                const { data, error } = await supabase
                    .from(IMAGES_TABLE)
                    .insert({ mixer_id: mixerId, image_url: `smyrna/${pathInBucket}`, created_at: nowIso() })
                    .select()
                    .maybeSingle()
                if (error) return errorResponse('Operation failed', headers, 400)
                return jsonResponse({ data }, headers)
            ...
            case 'delete-image': {
                ...
                if (imageData?.image_url) {
                    const relPath = imageData.image_url.startsWith('smyrna/')
                        ? imageData.image_url.substring('smyrna/'.length)
                        : imageData.image_url
