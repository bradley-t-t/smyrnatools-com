    ...
    try {
        ...
        switch (endpoint) {
            ...
            case 'fetch-operators': {
                const auth = await requireAuthenticated(supabase, req, headers)
                if (auth instanceof Response) return auth
                const [{ data: activeData, error: activeError }, { data: otherData, error: otherError }] =
                    await Promise.all([
            ...
            case 'create': {
                ...
                const row = {
                    employee_id:
                        typeof input?.employee_id === 'string' && input.employee_id
                            ? input.employee_id
                            : crypto.randomUUID(),
            ...
            case 'delete-comment': {
                ...
                const body = await parseBody(req)
                const commentId =
                    typeof body?.commentId === 'string' && body.commentId !== 'undefined' ? body.commentId : null
