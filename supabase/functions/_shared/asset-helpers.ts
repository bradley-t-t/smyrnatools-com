...
export function resolveChangedBy(body: any, req: Request): string {
    return typeof body?.changedBy === 'string' && body.changedBy
        ? body.changedBy
        : req.headers.get('X-User-Id') || SYSTEM_USER_ID
}
...
export async function handleAddComment(
    ...
    const { data, error } = await supabase
        .from(tables.comments!)
        .insert([{ [fkCol]: entityId, text, author, created_at: nowIso() }])
        .select()
        .maybeSingle()
    if (error) return errorResponse('Operation failed', headers, 400)
    ...
    try {
        ...
        if (plantCode) {
            ...
            const { data: authorProfile } = await supabase
                ...
                .eq('id', author)
                .maybeSingle()
            const commenterName = authorProfile
            ...
            fetch(`${baseUrl}/functions/v1/email-service/notify-comment-added`, {
                ...
                body: JSON.stringify({
                    ...
                    commentText: text,
                    assetType,
                    assetNumber: assetLabel,
                    plantCode
                })
            }).catch(() => {})
        }
    } catch {
        /* notification failure must never block the comment response */
    }
...
export async function handleAddHistory(
    supabase: any,
    body: any,
    req: Request,
    table: string,
...
export async function handleFetchIssues(
    supabase: any,
    body: any,
    table: string,
    idKey: string,
...
export async function handleCompleteIssue(
    supabase: any,
    body: any,
    req: Request,
    table: string,
...
export async function handleDeleteIssue(
    ...
    const issueId = typeof body?.issueId === 'string' ? body.issueId : null
    if (!issueId) return errorResponse('Issue ID is required', headers, 400)
    const { data: issue } = await supabase.from(table).select('id').eq('id', issueId).maybeSingle()
...
export async function handleFetchByField(
    supabase: any,
    body: any,
    table: string,
    field: string,
...
export async function handleSearchByField(
    supabase: any,
    body: any,
    table: string,
    field: string,
...
export async function handleFetchNeedingService(
    supabase: any,
    body: any,
    table: string,
    orderBy: string,
    ...
    return jsonResponse(
        {
            data: (data || []).filter((m: any) => !m.last_service_date || new Date(m.last_service_date) < thresholdDate)
        },
        headers
    )
}

export async function handleFetchCleanlinessHistory(
    supabase: any,
    body: any,
    table: string,
    idKey: string,
