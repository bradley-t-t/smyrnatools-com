    ...
    try {
        ...
        switch (endpoint) {
            ...
            case "fetch-items-with-profiles": {
                ...
                if (itemsError) return errorResponse(itemsError.message, headers, 400);
                ...
                if (idsSet.size) {
                    ...
                    if (profsError) return errorResponse(profsError.message, headers, 400);
                    profiles = profs ?? [];
                }
            ...
            case "add-planned-item": {
                ...
                if (error) return errorResponse(error.code === "23505" ? "Item already planned for this date" : error.message, headers, 400);
                return jsonResponse({success: true, id}, headers);
            }
